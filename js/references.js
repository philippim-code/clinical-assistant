...
(function(){
  ...
  function receiverPreviewEarMarkup(ear){
    ...
    if(receiverReady(ear))return `<div class="ref-ear-preview complete" data-preview-ear="${ear}"><img src="assets/spark/catalog/receivers/${assetSide}-${item.length}-${item.power}.png?v=1.8.0" alt="${receiverEarLabel(ear)} Spark receiver"><div class="ref-ear-preview-label"><span class="ref-side-marker${markerClass}" aria-hidden="true"></span>${receiverEarLabel(ear)}</div></div>`;
  ...
  function couplingAsset(ear){
    ...
    return `assets/spark/catalog/domes/${filename}.png?v=1.8.0`;
  }
