...
(function(){
  ...
  function installBilateralReceiver(){
    ...
    let both=row.querySelector('[data-receiver-both]');if(!both){both=document.createElement('button');both.type='button';both.className='ref-choice ref-choice-side';both.dataset.receiverBoth='1';both.innerHTML='<span class="ref-side-marker both" aria-hidden="true"></span><span>Both</span>';both.addEventListener('click',()=>{const live=referenceSection('Receivers'),active=live?.querySelector('[data-receiver-side].active')?.dataset.receiverSide;if(active)receiverSelections[active]={...currentReceiver(live)};receiverMode='both';installBilateralReceiver()});row.appendChild(both)}
