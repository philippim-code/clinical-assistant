...
(function(){
  ...
  function installRetentionGallery(){
    ...
    let gallery=section.querySelector('.ref-retention-gallery');if(!gallery){const names={S:'Small',M:'Medium',L:'Large'},files={S:'IMG_2176.png',M:'IMG_2177.png',L:'IMG_2178.png'};gallery=document.createElement('div');gallery.className='ref-retention-gallery';gallery.innerHTML=['S','M','L'].map(size=>`<button type="button" class="ref-retention-card" data-retention-card="${size}"><img src="assets/spark/catalog/retention-locks/${files[size]}?v=dev23" alt="${names[size]} Spark retention lock"><strong>${names[size]} (${size})</strong></button>`).join('');section.appendChild(gallery);gallery.querySelectorAll('[data-retention-card]').forEach(card=>card.addEventListener('click',()=>section.querySelector(`[data-retention="${card.dataset.retentionCard}"]`)?.click()))}
